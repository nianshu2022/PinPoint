...
5. 在 **Set up builds and deployments (构建和部署设置)** 环节：
   ...
   - ⚠️ **Environment variables (环境变量)**: （重要！）展开此项卡片，必须添加一项安全密钥变量，否则无法保持登录状态：
     - **Variable name**: `NUXT_SESSION_PASSWORD`
     - **Value**: （请随便随意填写一段至少 32 位的长字符串乱码，用于给会话 Token 加密用）
6. 点击 **Save and Deploy (保存并部署)**。

*（注：在点击部署的瞬间，隐藏在底层的 `@nuxthub/core` 核心会自动帮您在 Cloudflare 云端创建一个 D1 数据库并连接好一切资源，全程无需你动手写一句 SQL！）*

#### 第三步：如何修改环境变量 (供后续参考)
与普通 Docker 提供一个 `.env` 文件不同。在 Cloudflare 服务器上运作时所有的环境变量必须通过可视化面板输入。
如果你部署时忘记加变量，或后期想在后台手动添加额外变量时（比如不想通过向导输入而想硬编码某些设置）：
前往 Pages 项目页，在顶部点击 **Settings (设置)** -> **Environment variables (环境变量)** -> 在 **Production (生产环境)** 中添加你的变量，然后前往 **Deployments** 重新点击一次发布 (`Retry deployment`) 即可生效！
...
成功进入页面后，我们的第一步是**配置系统并上传照片**。

### 1. 初始化设置向导（全新安装必读）

得益于新版的自动检测机制，无论是通过 Cloudflare 部署还是私有云部署，当项目检测到数据库全新且为空时，您**首次访问您的域名，会自动引导进入系统初始化向导 (`/onboarding`)**。

请根据页面提示，一步步完成您的专属图库设置：
- **创建管理员账号**：在这里纯自定义您自己的邮箱和密码（无需再去改代码）。
- **站点偏好信息**：定义项目的图库名称、Slogan 甚至您的个人头像图片。
- **配置存储桶配置**：选择想要把照片存在哪（本地硬盘、AWS S3 兼容格式如腾讯云/阿里云、或者 Alist/OpenList 提供商）。

*(注：如果您使用的是带种子数据的旧版强制覆盖安装，遇到无法跳转的情况，仍可通过点击头像尝试输入后备默认账密 `admin@chronoframe.com` 及密码 `CF1234@!`)*

### 2. 开始登入并上传记录

1. 初始化向导完成后系统会进入前台，点击网页右侧的**用户头像**并使用刚才您创建好的账密登录。
